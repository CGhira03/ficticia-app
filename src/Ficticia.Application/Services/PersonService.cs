...
public class PersonService : IPersonService
{
    ...
    public async Task<PersonDto> CreateAsync(PersonDto dto)
    {
        ...
        var person = new Person
        {
            ...
            Attributes = dto.Attributes.Select(attr => new PersonAttribute
            {
                AttributeTypeId = _context.AttributeTypes
                    .FirstOrDefault(t => t.Type == attr.Type)?.Id
                    ?? throw new BusinessException($"El tipo de atributo '{attr.Type}' no existe."),
                Value = attr.Value
            }).ToList()
        };
    ...
    public async Task<PersonDto> UpdateAsync(int id, PersonDto dto)
    {
        ...
        {
            var typeEntity = await _context.AttributeTypes
                .FirstOrDefaultAsync(t => t.Type == attr.Type);
    ...
    private static PersonDto MapToDto(Person p) =>
        new PersonDto
        {
            ...
            Gender = p.Gender,
            IsActive = p.IsActive,
            Attributes = p.Attributes.Select(attr => new PersonAttributeDto
            {
                Type = attr.AttributeType?.Type ?? "(Sin tipo)",
                Value = attr.Value
            }).ToList()
        };
}
